...
def check_day_available(day, year):
...
def main(current_day, year):
    if 1 <= current_day <= 25:
        if not check_day_available(current_day, year):
            print_color(f"Day {current_day} is not available.", Fore.RED)
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--all",
        dest="all",
        help="fetch all currently available days",
        action="store_true",
    )
